import{_ as s,o as i,c as a,R as n}from"./chunks/framework.EUU6lLMq.js";const g=JSON.parse('{"title":"Как интегрировать KernelSU для не GKI ядер?","description":"","frontmatter":{},"headers":[],"relativePath":"ru_RU/guide/how-to-integrate-for-non-gki.md","filePath":"ru_RU/guide/how-to-integrate-for-non-gki.md"}'),t={name:"ru_RU/guide/how-to-integrate-for-non-gki.md"},e=n(`<h1 id="introduction" tabindex="-1">Как интегрировать KernelSU для не GKI ядер? <a class="header-anchor" href="#introduction" aria-label="Permalink to &quot;Как интегрировать KernelSU для не GKI ядер? {#introduction}&quot;">​</a></h1><p>KernelSU может быть интегрирован в ядра, отличные от GKI, и был перенесен на версии 4.14 и ниже.</p><p>В связи с фрагментацией ядер, отличных от GKI, у нас нет единого способа их сборки, поэтому мы не можем предоставить загрузочные образы, отличные от GKI. Однако вы можете собрать ядро самостоятельно с помощью интегрированной программы KernelSU.</p><p>Во-первых, вы должны уметь собирать загрузочное ядро из исходных текстов ядра. Если ядро не является открытым, то запустить KernelSU для вашего устройства будет затруднительно.</p><p>Если вы можете собрать загрузочное ядро, то существует два способа интеграции KernelSU в исходный код ядра:</p><ol><li>Автоматически с помощью <code>kprobe</code></li><li>Вручную</li></ol><h2 id="using-kprobes" tabindex="-1">Интеграция с kprobe <a class="header-anchor" href="#using-kprobes" aria-label="Permalink to &quot;Интеграция с kprobe {#using-kprobes}&quot;">​</a></h2><p>KernelSU использует kprobe для выполнения хуков ядра, если <em>kprobe</em> хорошо работает в вашем ядре, то рекомендуется использовать именно этот способ.</p><p>Сначала добавьте KernelSU в дерево исходных текстов ядра:</p><div class="language-sh vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">curl</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -LSs</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> &quot;https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh&quot;</span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;"> |</span><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;"> bash</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> -</span></span></code></pre></div><p>Затем необходимо проверить, включена ли функция <em>kprobe</em> в конфигурации ядра, если нет, то добавьте в нее эти настройки:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>CONFIG_KPROBES=y</span></span>
<span class="line"><span>CONFIG_HAVE_KPROBES=y</span></span>
<span class="line"><span>CONFIG_KPROBE_EVENTS=y</span></span></code></pre></div><p>И снова соберите ядро, KernelSU должен работать нормально.</p><p>Если вы обнаружите, что KPROBES по-прежнему не активирован, попробуйте включить <code>CONFIG_MODULES</code>. (Если это все равно не даст результата, используйте <code>make menuconfig</code> для поиска других зависимостей KPROBES).</p><p>Если же при интеграции KernelSU возникает зацикливание загрузки, то, возможно, в вашем ядре <em>kprobe неисправен</em>, следует исправить ошибку kprobe или воспользоваться вторым способом.</p><div class="tip custom-block"><p class="custom-block-title">Как проверить, не сломан ли kprobe？</p><p>закомментируйте <code>ksu_enable_sucompat()</code> и <code>ksu_enable_ksud()</code> в файле <code>KernelSU/kernel/ksu.c</code>, если устройство загружается нормально, то может быть нарушена работа kprobe.</p></div><h2 id="modify-kernel-source-code" tabindex="-1">Ручная модификация исходного кода ядра <a class="header-anchor" href="#modify-kernel-source-code" aria-label="Permalink to &quot;Ручная модификация исходного кода ядра {#modify-kernel-source-code}&quot;">​</a></h2><p>Если kprobe не работает в вашем ядре (возможно, это ошибка апстрима или ядра ниже 4.8), то можно попробовать следующий способ:</p><p>Сначала добавьте Kernel SU в дерево исходного кода вашего ядра:</p><ul><li>Последний тэг(стабильный)</li></ul><div class="language-sh vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">curl</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -LSs</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> &quot;https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh&quot;</span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;"> |</span><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;"> bash</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> -</span></span></code></pre></div><ul><li>Основная ветвь(разработка)</li></ul><div class="language-sh vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">curl</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -LSs</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> &quot;https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh&quot;</span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;"> |</span><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;"> bash</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -s</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> main</span></span></code></pre></div><ul><li>Выбранный тэг(Например, версия v0.5.2)</li></ul><div class="language-sh vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">sh</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">curl</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -LSs</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> &quot;https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh&quot;</span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;"> |</span><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;"> bash</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> -s</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> v0.5.2</span></span></code></pre></div><p>Затем добавьте вызовы KernelSU в исходный код ядра, вот патч, на который можно сослаться:</p><div class="language-diff vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">diff --git a/fs/exec.c b/fs/exec.c</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> 		return -EINVAL;</span></span></code></pre></div><p>В исходных кодах ядра можно найти эти четыре функции:</p><ol><li>do_faccessat, обычно в <code>fs/open.c</code>.</li><li>do_execveat_common, обычно в <code>fs/exec.c</code>.</li><li>vfs_read, обычно в <code>fs/read_write.c</code>.</li><li>vfs_statx, обычно в <code>fs/stat.c</code>.</li></ol><p>Если в вашем ядре нет <code>vfs_statx</code>, используйте вместо него <code>vfs_fstatat</code>:</p><div class="language-diff vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">diff --git a/fs/stat.c b/fs/stat.c</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> 		goto out;</span></span></code></pre></div><p>Для ядер младше 4.17, если вы не можете найти <code>do_faccessat</code>, просто перейдите к определению системного вызова <code>faccessat</code> и поместите вызов туда:</p><div class="language-diff vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">diff --git a/fs/open.c b/fs/open.c</span></span>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#85E89D;">+</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> 	if (mode &amp; ~S_IRWXO)	/* where&#39;s F_OK, X_OK, W_OK, R_OK? */</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> 		return -EINVAL;</span></span></code></pre></div><p>Чтобы включить встроенный в KernelSU безопасный режим, необходимо также изменить <code>input_handle_event</code> в файле <code>drivers/input/input.c</code>:</p><div class="tip custom-block"><p class="custom-block-title">TIP</p><p>Настоятельно рекомендуется включить эту функцию, она очень помогает предотвратить циклическую загрузку!</p></div><div class="language-diff vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">diff</span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">diff --git a/drivers/input/input.c b/drivers/input/input.c</span></span>
...
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;"> 		add_input_randomness(type, code, value);</span></span></code></pre></div><p>Наконец, снова соберите ядро, KernelSU должен работать хорошо.</p>`,40),l=[e];function p(h,k,d,r,c,E){return i(),a("div",null,l)}const f=s(t,[["render",p]]);export{g as __pageData,f as default};
